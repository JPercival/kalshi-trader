...
export function getOpenTrades(db) {
  return db.prepare("SELECT pt.*, m.event_ticker, m.series_ticker FROM paper_trades pt LEFT JOIN markets m ON pt.ticker = m.ticker WHERE pt.resolution = 'open' ORDER BY pt.opened_at DESC").all();
}
...
export function getAllTrades(db, { limit = 100 } = {}) {
  return db.prepare("SELECT pt.*, m.event_ticker, m.series_ticker FROM paper_trades pt LEFT JOIN markets m ON pt.ticker = m.ticker ORDER BY pt.opened_at DESC LIMIT ?").all(limit);
}
