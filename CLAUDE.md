...
# Kalshi Paper Trader — Agent Instructions

## Project Overview
Prediction market paper trader. Monitors Kalshi markets, builds probabilistic models to detect mispricings, simulates trades with Quarter-Kelly sizing. See VISION.md for full spec.

## Code Quality Standards

### 100% Test Coverage — Non-Negotiable
- **Every module must have corresponding tests** in a `__tests__/` directory or `*.test.js` co-located file
- **Coverage target: 100% line and branch coverage**
- Run tests with `npm test` (uses Node.js built-in test runner or vitest — pick one and stick with it)
- Run coverage with `npm run test:coverage`
- **Do not mark a task as Done unless tests exist and coverage is 100% for new/modified files**
- Test external API calls with mocks/stubs — never hit real APIs in tests

### Code Style
- Node.js, ES modules (`import`/`export`)
- No TypeScript (keep it simple, this is a validation tool)
- Use `dotenv` for config, `better-sqlite3` for DB, `express` + `ejs` for web
- Descriptive variable names, JSDoc comments on public functions
- Small functions, single responsibility

### Commit Discipline
- Commit after each task completion
- Meaningful commit messages: `feat:`, `fix:`, `test:`, `docs:` prefixes
- One logical change per commit

## Key Files
- `VISION.md` — Product spec, data model, success criteria, API reference
- `backlog/` — Task tracking (use `backlog` CLI, never edit task files directly)
- `.env.example` — All config vars with descriptions

## Architecture Notes
- **Two-tier polling:** Market ingestion (every 60s) + model runs (every 5min)
- **4 probability models:** Weather (NOAA), Economics (FRED), Fed rates, Base rates
- **Position sizing:** Quarter-Kelly, capped at MAX_POSITION_PCT per trade
- **Dashboard:** Express + EJS + Chart.js on port 3001
